...
def pm_G(input_nc, output_nc, ngf, n_stg=3, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], n_phy=1, phyMode='concat',
    ...
    :return:
    '''
    # this one wrapp the net and init. But I think not necessary as init not always studied, just tune with a good one.
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    ## single version
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        self.register_buffer('real_label', torch.tensor(target_real_label))     # attribute but not as parameters
...
class autoWtL(torch.nn.Module):
    def __init__(self, whtScal =100, clipMod='clip01', baseWht=1, type_L = 'L2'):
        ...
        self.baseWht = baseWht
        self.type_L = type_L
    def forward(self, src, tar, **kwargs):      # kw for additional input
        if self.clipMod == 'clip11':
            wht = (tar+1) / 2 * self.whtScal + self.baseWht
        elif self.clipMod == 'clip01':
            wht = tar * self.whtScal + self.baseWht
...
class UnetGenerator(nn.Module):
    ...
    # use a conv + relu+ norm strucutr, so  conv always -1 to 1
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):       # num downs = 7 so 2x2 finally
        ...
        unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer, follow nets are wrapped up around current bone by adding head conv and tail deconv, the innermost 3 above 5 will use same ngf.  (number of gan features? )
...
class StkPmGenerator(nn.Module):
    '''
    generate stackeed pm generator. Each moduel generate output also the ngf feature maps.  input, current output and fts will be combined as input for next stage. stacked wrapper
    '''
...
class UnetPmGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_phy=1, phyMode= 'concat', n_gateLayers=1,actiType=None, sz_std=256):
        ...
        self.n_phy = n_phy      # input number of phy
        self.phyMode = phyMode
        self.n_gateLayers = n_gateLayers
        self.ngf = ngf
        self.n_phyCode = 3      #  fixed  6  code, assume head, shoulder, hip, elbow, heel.and torso in general
        # construct unetPm
        # down branch
        down_li = nn.ModuleList()
        down_li.append(downBlk(input_nc,ngf,outermost=True, norm_layer=norm_layer)) # n_d = 1
        ngf_tmp = ngf       # record current  ch
        if num_downs<2:
            print('num_downs at least 2 layers but get', num_downs)
            quit(-1)
        for i in range(min(3, num_downs-2)):  # upscale ngf to 8      # n_d4,  minus exclude 1st and last specific layer
        ...
        elif 'enc' == phyMode and self.n_phy>0:      # if us encoder
            n_encUp = ngf_tmp+self.n_phyCode     # add fixed length that, for inner most n_features
            # pwrs add the two layer fc
            n_pc = self.n_phyCode
            li_fc = nn.ModuleList()
            li_fc.append(nn.Linear(n_phy, 10))       # simplify the layers
            # li_fc.append(nn.BatchNorm1d(10))
            li_fc.append(nn.ReLU(True))
            li_fc.append(nn.Dropout(0.5))
            li_fc.append(nn.Linear(10, 10))  # simplify the layers
            # li_fc.append(nn.BatchNorm1d(10))
            li_fc.append(nn.ReLU(True))
            li_fc.append(nn.Dropout(0.5))
            # li_fc.append(nn.Linear(10, 10))
            li_fc.append(nn.Linear(10, n_pc))     # n_phyCode 6
            # li_fc.append(nn.BatchNorm1d(n_pc))
            # li_fc.append(nn.ReLU(True))
            self.phyNet = nn.Sequential(*li_fc)
        else:
            n_encUp = ngf_tmp
        up_li.append(upBlk(n_encUp, ngf_tmp, innermost=True, norm_layer=norm_layer))    # n_up = 1
        # same 8ngf section
        for i in range(num_downs-5):        # n_up = 4 (8)   beond 5 part
            up_li.append(upBlk(ngf_tmp*2, ngf_tmp, norm_layer=norm_layer, use_dropout=use_dropout))
        for i in range(min(3, num_downs-2)):  # down ngf section  # n_up = 7    3 expanding ch part
        ...
        # abandoned gate structure
        # if 'fc_gated' == phyMode: # add gated fc layers, single layer now. featurewise gate
    ...
    def forward(self, img, phyVec):
        ...
        elif 'enc' == self.phyMode and self.n_phy>0:    # if there is code
            # elif self.n_phy>0:
            phyCode = self.phyNet(phyVec[:, :self.n_phy])   # the code length
            y = torch.cat((y, phyCode.view(-1, self.n_phyCode, 1, 1)), 1)
        # abandoned gatted structure
        ...
        for i, module in enumerate(self.up_li):
            ...
            y = module(y)       #  weight of size 518 512 4 4, expected input[70, 513, 1, 1]
